    ...
    public class ImageVerifier : IImageVerifier
    {
        ...
        public bool IsImage(byte[] bytes)
        {
            return ImageFormatDecoders.Keys.Any(
                    ImageFormatDecoders => ImageFormatDecoders.SequenceEqual(bytes.Take(ImageFormatDecoders.Length))
                );
        }
        ...
        private static bool IsNmp(byte[] bytes)
        {
            if (bytes.Length < 2)
                return false;
