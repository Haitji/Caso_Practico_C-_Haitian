...
namespace Bootcamp_store_backend.Infrastructure.Specs
{
    public class Specification<T>
    {
            ...
            foreach (var criterion in _criteria)
            {
                ...
                switch(criterion.Operator)
                {
                    ...
                    case "MATCH":
                        ...
                        break;
                    case "MATCH_START":
                        predicate = Expression.Call(ToCaseInsensitiveStringExpression(property), "StartsWhith", null, ToCaseInsensitiveStringExpression(constant));
                        break;
                    case "MATCH_END":
        ...
        public Expression ToCaseInsensitiveStringExpression(Expression expression)
        {
            if ( expression.Type != typeof(string))
            {
                var toStringMethod = expression.Type.GetMethod("ToString", Array.Empty<Type>()) ?? throw new MalfomedFilterException();
                expression = Expression.Call(expression, toStringMethod);

            }
            var toLowerMethod = typeof(string).GetMethod("ToLower", Array.Empty<Type>()) ?? throw new MalfomedFilterException();
            expression = Expression.Call(expression, toLowerMethod);

            return expression;
        }

        public MemberExpression GetProperty(Expression instance, string[] propertyNames, int index = 0)
        {
